...
namespace Infrastructure.Repositories.Dashboard
{
    public class DashboardMilkCouponRepository : IDashboardMilkCouponRepository
    {
        ...
        {
            DataTable dtResult = await _sqlProvider.FromQueryAsync(string.Format("exec PR_DASHBOARD_MILKCOUPON_BY_DAYFROMTO {0},{1}", dayFrom, dayTo));
        ...
        {
            DataTable dtResult = await _sqlProvider.FromQueryAsync(string.Format("exec PR_DASHBOARD_MILKCOUPON_BY_DAYFROMTO_FARMER {0},{1},{2}", dayFrom, dayTo, farmerId));
            IEnumerable<DashboardMilkCouponModel> results = IMapperExtentions.ConvertDataTable<DashboardMilkCouponModel>(dtResult).ToList();
            return results;
        }
    }
